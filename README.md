...
| <a name="input_allowed_to_merge"></a> [allowed\_to\_merge](#input\_allowed\_to\_merge) | Array of access levels and user(s)/group(s) allowed to merge to protected branch. Only available for Premium and Ultimate instances. | ```list(object({ access_level = optional(string) group_id = optional(number) user_id = optional(number) }))``` | `[]` | no |
| <a name="input_allowed_to_push"></a> [allowed\_to\_push](#input\_allowed\_to\_push) | Array of access levels and user(s)/group(s)/deploy keys allowed to push to protected branch. Only available for Premium and Ultimate instances. | ```list(object({ access_level = optional(string) deploy_key_id = optional(number) group_id = optional(number) user_id = optional(number) }))``` | `[]` | no |
